...
def main():
    ...
    gray_tempplates = []
    for i in react:
        str = 'C:\\Users\\THONG\\Desktop\\Facebook-auto-react-OpenCV-master\\Facebook-auto-react-OpenCV-master\\Fb auto react OpenCV\\template\\'
        str += i
        templates = cv2.imread(str)
        temp=cv2.cvtColor(templates, cv2.COLOR_BGR2GRAY )
        gray_tempplates.append(temp)

    gray_reactions_haha = []
    for i in reactions_haha:
        str = 'C:\\Users\\THONG\\Desktop\\Facebook-auto-react-OpenCV-master\\Facebook-auto-react-OpenCV-master\\Fb auto react OpenCV\\template\\'
        str += i
        reaction = cv2.imread(str)
        temp=cv2.cvtColor(reaction, cv2.COLOR_BGR2GRAY)
        gray_reactions_haha.append(temp)
    ...
    for i in reactions_like:
        ...
        reaction = cv2.imread(str)
        temp=cv2.cvtColor(reaction, cv2.COLOR_BGR2GRAY)
        gray_reactions_like.append(temp)
    ...
    for i in reactions_love:
        ...
        reaction = cv2.imread(str)
        temp=cv2.cvtColor(reaction, cv2.COLOR_BGR2GRAY)
        gray_reactions_love.append(temp)
    ...
    for i in reactions_sad:
        ...
        reaction = cv2.imread(str)
        temp=cv2.cvtColor(reaction, cv2.COLOR_BGR2GRAY)
        gray_reactions_sad.append(temp)
    ...
    for i in reactions_wow:
        ...
        reaction = cv2.imread(str)
        temp=cv2.cvtColor(reaction, cv2.COLOR_BGR2GRAY)
        gray_reactions_wow.append(temp)
        ...
        for gray_tempplate in gray_tempplates:
            ...
            for i in zip(*loc[::-1]):
                ...
                if react_gray_tempplate_index == 0:
                    ...
                    cv2.putText(screen_record, "chua like", (i[0] + w + 30, i[1] - 24), font, 0.65, (255,51,255), cv2.LINE_AA)
                    ...
                    if react_gray_tempplate_index == 1:
                        cv2.line(screen_record, (i[0] + w + 30, i[1] - 15), (i[0] + w + 90, i[1] - 15), (0,255,0), 2)
                        cv2.putText(screen_record, 'like roi', (i[0] + w + 30, i[1] - 24), font, 0.65, (255,51,255), cv2.LINE_AA)
                    else:
                        # highlight the loved or heart status
                        if react_gray_tempplate_index == 2:
                            cv2.line(screen_record, (i[0] + w + 30, i[1] - 15), (i[0] + w + 90, i[1] - 15), (0,255,0), 2)
                            cv2.putText(screen_record, 'da tha tim', (i[0] + w + 30, i[1] - 24), font, 0.65, (255,51,255), cv2.LINE_AA)
                        else:
                            # highlight the funny status
                            if react_gray_tempplate_index == 3:
                                cv2.line(screen_record, (i[0] + w + 30, i[1] - 15), (i[0] + w + 90, i[1] - 15), (0,255,0), 2)
                                cv2.putText(screen_record, 'tha haha', (i[0] + w + 30, i[1] - 24), font, 0.65, (255,51,255), cv2.LINE_AA)
                            else:
                                # highlight the surprise status
                                if react_gray_tempplate_index == 4:
                                    cv2.line(screen_record, (i[0] + w + 30, i[1] - 15), (i[0] + w + 90, i[1] - 15), (0,255,0), 2)
                                    cv2.putText(screen_record, 'ngac nhien chua', (i[0] + w + 30, i[1] - 24), font, 0.65, (255,51,255), cv2.LINE_AA)
                                else:
                                    # highlight the sad status
                                    if react_gray_tempplate_index == 5:
                                        cv2.line(screen_record, (i[0] + w + 30, i[1] - 15), (i[0] + w + 90, i[1] - 15), (0,255,0), 2)
                                        cv2.putText(screen_record, 'buon ghe', (i[0] + w + 30, i[1] - 24), font, 0.65, (255,51,255), cv2.LINE_AA)
                                    else:
                                        # highlight the status that makes people angry
                                        if react_gray_tempplate_index == 6:
                                            cv2.line(screen_record, (i[0] + w + 30, i[1] - 15), (i[0] + w + 90, i[1] - 15), (0,255,0), 2)
                                            cv2.putText(screen_record, 'tuc qua ma', (i[0] + w + 30, i[1] - 24), font, 0.65, (255,51,255), cv2.LINE_AA)
